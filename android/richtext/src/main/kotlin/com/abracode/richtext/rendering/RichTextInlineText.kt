...
/** The built inline string plus the inline images it references (empty for the common text-only case). */
...
fun buildRichInlines(
    ...
    val text = buildAnnotatedString {
        appendInlines(nodes, InlineContext(initialBold, italic = false, strike = false, link = null, color = baseColor, colors = colors), images)
    }
    return RichTextInlineResult(text, images)
}
...
private fun AnnotatedString.Builder.appendInlines(
    ...
    for (node in nodes) {
        when (node) {
            ...
            is RichTextInline.Code -> appendCodeRun(node.value, ctx)
            is RichTextInline.Emphasis -> appendInlines(node.children, ctx.copy(italic = true), images)
            is RichTextInline.Strong -> appendInlines(node.children, ctx.copy(bold = true), images)
            is RichTextInline.Strikethrough -> appendInlines(node.children, ctx.copy(strike = true), images)
            is RichTextInline.Link -> {
                ...
                if (RichTextURLPolicy.allowedLink(node.url) != null) {
                    withLink(LinkAnnotation.Url(node.url)) {
                        appendInlines(node.text, ctx.copy(link = node.url), images)
                    }
                } else {
                    appendInlines(node.text, ctx, images)
                }
            }
...
private fun AnnotatedString.Builder.appendCodeRun(s: String, ctx: InlineContext) {
    // Inline code: monospace + a filled background (phase-1 square span; Stage E draws the rounded pill).
    // No underline even inside a link, matching the Swift codeRun. fontWeight is pinned to Normal so a code span
    // inside a heading (whose base style is Bold) renders regular-weight mono like Swift's unconditional monoFont.
    withStyle(
        SpanStyle(
            color = if (ctx.link != null) ctx.colors.link else ctx.color,
            ...
            fontSize = 0.94.em,
            background = ctx.colors.codeFill,
